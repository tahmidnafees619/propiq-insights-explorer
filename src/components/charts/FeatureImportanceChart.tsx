  ...
  return (
    <div className="card-surface gradient-top-border p-5">
      ...
      <div style={{ width: '100%', height: 280 }}>
        <ResponsiveContainer>
          <BarChart data={top} layout="vertical" margin={{ top: 5, right: 50, left: 10, bottom: 5 }}>
            <XAxis type="number" hide />
            <YAxis dataKey="label" type="category" stroke="#475569" tick={{ fill: '#94A3B8', fontSize: 11 }} width={120} />
            <Tooltip content={<DarkTooltip />} cursor={{ fill: 'rgba(59,130,246,0.08)' }} />
            <Bar dataKey="importance" radius={[0, 8, 8, 0]} animationDuration={700}>
              {top.map((_, i) => <Cell key={i} fill={i < 3 ? '#10B981' : '#3B82F6'} />)}
              <LabelList dataKey="importance" position="right" formatter={(v: any) => `${(v * 100).toFixed(1)}%`} style={{ fill: '#94A3B8', fontSize: 11 }} />
            </Bar>
          </BarChart>
        </ResponsiveContainer>
      </div>
