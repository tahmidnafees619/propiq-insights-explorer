...
type SortKey = keyof Pick<
  Property,
  "price" | "sqft_living" | "bedrooms" | "bathrooms" | "grade" | "condition" | "yr_built"
>;
...
export function PropertyTable() {
  ...
  const priceColor = (p: number) => {
    if (p > median * 1.25) return "#3DAE91";
    if (p < median * 0.75) return "#D5533F";
    return "#D0A74E";
  };
      ...
      <div className="flex items-center justify-between mb-4 gap-4 flex-wrap">
        <div>
          ...
          <p className="text-xs text-muted-foreground">
            {fmtNumber(filtered.length)} properties · sorted by {sort.key}
          </p>
        </div>
        <div className="relative">
          ...
          <input
            ...
            className="bg-[#0C1318] border border-[#28363E] rounded-xl pl-9 pr-3 py-2 text-xs text-foreground placeholder:text-muted-foreground focus:outline-none focus:border-[#2F99DA] transition w-64"
          />
        </div>
      </div>
        ...
        <table className="w-full text-xs">
          <thead>
            <tr className="text-muted-foreground border-b border-[#28363E]">
              {cols.map((c) => (
                <th
                  key={c.key}
                  className={`text-left py-2.5 px-3 font-medium uppercase tracking-wider text-[10px] ${c.sortable ? "cursor-pointer hover:text-foreground group" : ""}`}
          ...
          <tbody>
            {pageRows.map((p, idx) => (
              <tr
                key={p.id}
                className="border-b border-[#28363E]/50 hover:bg-[#192329]/60 transition border-l-2 border-l-transparent hover:border-l-[#2F99DA]"
                style={{ background: idx % 2 ? "transparent" : "rgba(25,35,41,0.3)" }}
              >
                <td className="py-2.5 px-3 font-semibold" style={{ color: priceColor(p.price) }}>
                  {fmtCurrency(p.price)}
                </td>
                ...
                <td className="py-2.5 px-3">
                  <span className="px-2 py-0.5 rounded-md bg-[#28363E] text-[10px] font-medium text-foreground">
                    {p.grade}
                  </span>
                </td>
            ...
            {pageRows.length === 0 && (
              <tr>
                <td colSpan={cols.length} className="py-12 text-center text-muted-foreground">
                  {isLoading ? "Loading properties…" : "No properties match your filter"}
                </td>
              </tr>
            )}
          </tbody>
      ...
      <div className="flex items-center justify-between mt-4 text-xs text-muted-foreground">
        ...
        <div className="flex gap-2">
          <button
            ...
            disabled={page === 1}
            className="px-3 py-1.5 rounded-lg border border-[#28363E] hover:bg-[#192329] disabled:opacity-40 transition btn-press inline-flex items-center gap-1"
          >
